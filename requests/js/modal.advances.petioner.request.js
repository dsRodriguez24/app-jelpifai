const get_advances_by_request_petioner = async (id_encrypted) => {
    try {

        const id    = atob(id_encrypted);
        const url   = `${BASE_URL_NODE_SERVER}solicitud_oferta_evidencia/solicitud/${id}/solicitante`;
        ...
        add_advances_petioner_to_modal(data);
...
const add_advances_petioner_to_modal = (data) => {

    let html                            = '<p>Aun no hay avances. Consulta en las proximas horas...</p>';
    const btn_aceptar_cerrar_advances   = document.querySelector('#modal-advances-petitioner-request #btn-aceptar-cerrar-advances-petitioner');
    ...
    if (data.length > 0) {
        html = data.map( ( proposal, index ) =>  {
            ...
            if (evidencia_descartada == '0') {
                ...
                button_descartar_avance = `<a title="Descartar avance" onclick="discard_advance_petitioner('${btoa(proposal.id)}')" class="btn btn-icon btn-sm  btn-danger text-white mx-1 my-1" style="opacity: 0.7"> <i class='fas fa-xmark'></i></a>`;
            }
    ...
    $("#content-modal-advances-petioner").html(html);
    $("#modal-advances-petitioner-request").modal('show');
}
...
const update_request_confirm_applicant = async (solicitud_id) => {
    ...
    const { value: swal_confirm} = await Swal.fire({
        ...
        text: "Asegurese de estar conforme con la entrega del solicitante.",
        icon: 'warning',
    ...
    try {
        ...
        const url       = `${BASE_URL_NODE_SERVER}solicitud/finalizar_por_voluntario/${solicitud_id}`;
        const token     = localStorage.getItem('token');
        const res   = await fetch(url, {
            method: "PUT",
            headers: {
...
const discard_advance_petitioner = async (encrypted_advance_id) => {
    try {

        $("#modal-advances-petitioner-request").modal('hide');
        ...
        await get_advances_by_request_petioner( btoa(data.solicitud_id) );
        return;
